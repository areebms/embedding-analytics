...
def test_terms_lists_multi_book_terms(client, patch_storage):
...
def test_terms_excludes_pos_tag_r(client, patch_storage):
...
def test_terms_response_validates_against_schema(client, patch_storage):
    _, term_table = patch_storage
    _set_term_entries(term_table, {
...
def test_terms_books_field_contains_platform_data_ids(client, patch_storage):
    _, term_table = patch_storage
    _set_term_entries(term_table, {
        "gutenberg-1": [{"term": "labour", "tags": {"N"}}],
        "gutenberg-2": [{"term": "labour", "tags": {"N"}}],
...
def test_terms_empty_corpus(client, patch_storage):
