...
def test_books_lists_aligned(client, patch_storage):
...
def test_books_excludes_unaligned(client, patch_storage):
    pipeline_table, _ = patch_storage
    custom = [
...
def test_books_response_validates_against_schema(client, patch_storage):
    body = client.get("/books").json()
...
def test_books_label_format(client, patch_storage):
...
def test_books_label_single_name_author(client, patch_storage):
...
def test_books_empty(client, patch_storage):
    pipeline_table, _ = patch_storage
...
def test_books_preserves_fields(client, patch_storage):
    pipeline_table, _ = patch_storage
