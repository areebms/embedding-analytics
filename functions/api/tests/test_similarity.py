...
def test_similarity_happy_path(client, patch_storage):
...
def test_similarity_response_validates_against_schema(client, patch_storage):
    _, term_table = patch_storage
...
def test_similarity_skips_r_tagged_terms(client, patch_storage):
...
def test_similarity_compound_expression(client, patch_storage):
    _, term_table = patch_storage
    rows = {
        "labour": make_term_entry("labour", seed=1),
        "value": make_term_entry("value", seed=2),
...
def test_similarity_unknown_term_returns_404(client, patch_storage):
    _, term_table = patch_storage
...
def test_similarity_invalid_payload_returns_422(client, patch_storage):
    response = client.post("/similarity/1", json={})
    assert response.status_code == 422


def test_similarity_invalid_op_returns_422(client, patch_storage):
    response = client.post(
        "/similarity/1",
...
def test_similarity_blank_term_returns_422(client, patch_storage):
